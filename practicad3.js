...
function drawMap(featureCollection) { 
    ...
    const projection = d3.geoMercator()
       .fitSize([width/2 ,height], featureCollection)
       .center(center)
       .translate([width/4, height/2 + 100]);		// Punto central del mapa   
...
*/
    ...
    neighbourhoodsPath.attr('fill', (d) => color(d.properties.avgprice))
	  ...
	  for (let index = 0; index < maxavgprice; index += maxavgprice/numberOfLegends) {
		   ...
		   const widthRect = (width/2*numberOfLegends) -2;
		   rectColor
...
/*
    const svggraph = d3.select('#graphd3')
      .append('svggraph');

    console.log(svggraph);
    console.log('svg graphd3');
    svg.attr('width', width/2)
      .attr('height', height)
      .attr('transform', `translate (${width/2}, ${width})`);
